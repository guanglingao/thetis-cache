...
public class KeyGenerator {
    ...
    public static String getCacheBreakKey(ProceedingJoinPoint thisJoinPoint) {
        try {
            ...
            CacheBreak annoCache = objMethod.getAnnotation(CacheBreak.class);
            if (annoCache != null) {
                String modelName = annoCache.model().getName();
                String key = annoCache.key();
                ...
                if (key.equals("")) {
                    ...
                    keyStrBuffer.append(")");
                    key = keyStrBuffer.toString();
                } else {
                    key = getDynamicVal(thisJoinPoint, key);
                }
    ...
    public static String getCacheThisKey(ProceedingJoinPoint thisJoinPoint) {
        try {
            ...
            Class<?>[] params = ((MethodSignature) thisJoinPoint.getSignature()).getParameterTypes();
            Method objMethod = classTarget.getMethod(methodName, params);
            CacheThis annoCache = objMethod.getAnnotation(CacheThis.class);
            if (annoCache != null) {
                String modelName = annoCache.model().getName();
                String key = annoCache.key();
                if (modelName.equals("DefaultModel")) {
    ...
    private static Object getFieldValueByName(String fieldName, Object o) {
        if (!fieldName.contains(".")) {
            ...
            try {
                ...
                value = getFieldByClasss(fieldName, o);
            }
            return value;
        } else {
    ...
    private static Object getFieldByClasss(String fieldName, Object object) {
        try {
